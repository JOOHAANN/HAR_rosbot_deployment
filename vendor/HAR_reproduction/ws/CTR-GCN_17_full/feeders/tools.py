...
def random_choose(data_numpy, size, auto_pad=True):
    # input: C,T,V,M; randomly selects a segment, which is unreliable since padded frames contain zeros
    C, T, V, M = data_numpy.shape
    if T == size:
        return data_numpy
    elif T < size:
...
def random_flip(data_numpy, p=0.5):
    """Mirror a Kinect skeleton around the x-axis and swap left/right joints."""
    if random.random() >= p:
        return data_numpy

    flipped = data_numpy.copy()
    flipped[0] = -flipped[0]
    # NTU/Kinect 25-joint left/right pairs, converted to 0-based indices.
    swap_pairs = (
        (4, 8), (5, 9), (6, 10), (7, 11),
...
def random_noise(data_numpy, std=0.01):
    """Add small Gaussian noise to valid skeleton coordinates only."""
    if std <= 0:
        return data_numpy

    noisy = data_numpy.astype(np.float32, copy=True)
    valid_mask = np.abs(noisy).sum(axis=0, keepdims=True) > 0
    noise = np.random.normal(0.0, std, size=noisy.shape).astype(np.float32, copy=False)
    noisy += noise * valid_mask
    return noisy
