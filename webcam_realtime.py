...
def build_arg_parser(description="Run CLIPGCN realtime inference from a local webcam."):
    ...
    parser.add_argument(
        "--rtmpose-det",
        default=None,
        help="Optional local RTMPose detector ONNX file. When set with --rtmpose-pose, no model download is attempted.",
    )
    parser.add_argument(
        "--rtmpose-pose",
        default=None,
        help="Optional local RTMPose pose ONNX file. When set with --rtmpose-det, no model download is attempted.",
    )
    parser.add_argument(
        "--rtmpose-device",
        default=None,
...
class RTMPoseSource:
    ...
    def __init__(self, args):
        ...
        detector = getattr(args, "rtmpose_det", None)
        pose = getattr(args, "rtmpose_pose", None)
        if bool(detector) != bool(pose):
            raise ValueError("--rtmpose-det and --rtmpose-pose must be provided together.")
        if detector and pose:
            detector = str(resolve_existing_path(detector, label="RTMPose detector"))
            pose = str(resolve_existing_path(pose, label="RTMPose pose"))
            detector_sizes = {
                "lightweight": (416, 416),
                "balanced": (640, 640),
                "performance": (640, 640),
            ...
            body_kwargs.update(
                {
                    "det": detector,
                    "det_input_size": detector_sizes[self.mode],
                    "pose": pose,
                    "pose_input_size": pose_sizes[self.mode],
                }
            )
            print(f"  rtmpose_models: detector={detector} pose={pose}")
        self.body = Body(**body_kwargs)
...
def build_prediction_from_scores(score_result, args, candidate_labels, temporal_mode):
    ...
    return {
        ...
        # Keep the complete candidate bank for ROS-side two-robot fusion.
        # The GUI still displays only top-k, while the fusion node receives
        # the same adjusted ranking scores that produced that top-k result.
        "candidate_labels": [int(label) for label in candidate_labels],
        "candidate_scores": [
            float(value) for value in score_result["cosine_scores"].detach().cpu().tolist()
        ],
        "candidate_ranking_scores": [
            float(value)
            for value in score_result["prediction_scores"].detach().cpu().tolist()
        ],
        "candidate_raw_scores": [
            float(value) for value in score_result["raw_cosine_scores"].detach().cpu().tolist()
        ],
...
def publish_ros_prediction(prediction, args, capture):
    """Publish one complete prediction event when the capture is ROS-backed.

    ``webcam_realtime.py`` is also used by the local webcam demo, so the
    publisher is intentionally discovered from the capture object rather than
    making ROS a dependency of that path. The event is JSON so the
    coordinator/fusion process remains independent of this Python model code.
    """

    publisher = getattr(capture, "prediction_publisher", None)
    if publisher is None:
        return

    from std_msgs.msg import String

    sequence = int(getattr(capture, "prediction_sequence", 0)) + 1
    capture.prediction_sequence = sequence
    candidate_labels = [int(value) for value in prediction.get("candidate_labels", [])]
    candidate_scores = [
        float(value) for value in prediction.get("candidate_scores", [])
    ]
    candidate_ranking_scores = [
        float(value) for value in prediction.get("candidate_ranking_scores", [])
    ]
    candidate_raw_scores = [
        float(value) for value in prediction.get("candidate_raw_scores", [])
    ]
    if not (
        len(candidate_labels)
        == len(candidate_scores)
        == len(candidate_ranking_scores)
        == len(candidate_raw_scores)
    ):
        raise ValueError("prediction candidate arrays have inconsistent lengths")

    event = {
        "schema": "har.vpoclip.prediction.v1",
        "robot_namespace": getattr(args, "robot_namespace", None),
        "sequence": sequence,
        "stamp_sec": time.time(),
        "temporal_mode": prediction.get("temporal_mode", "13f"),
        "labels": [int(value) for value in prediction.get("labels", [])],
        "scores": [float(value) for value in prediction.get("scores", [])],
        "ranking_scores": [
            float(value) for value in prediction.get("ranking_scores", [])
        ],
        "candidate_labels": candidate_labels,
        "candidate_scores": candidate_scores,
        "candidate_ranking_scores": candidate_ranking_scores,
        "candidate_raw_scores": candidate_raw_scores,
        "unknown": bool(prediction.get("unknown", False)),
        "analysing": bool(prediction.get("analysing", False)),
        "gate": prediction.get("gate") or {},
        "unknown_entropy": float(prediction.get("unknown_entropy", 0.0)),
        "unknown_top1_score": float(prediction.get("unknown_top1_score", 0.0)),
        "elapsed": float(prediction.get("elapsed", 0.0)),
        "end_to_end_elapsed": float(
            prediction.get("end_to_end_elapsed", prediction.get("elapsed", 0.0))
        ),
        "detected_pose_frames": int(prediction.get("detected_pose_frames", 0)),
        "person_present": getattr(args, "current_person_present", None),
    }
    message = String()
    message.data = json.dumps(event, ensure_ascii=False, separators=(",", ":"))
...
FUSION_DISPLAY_COLOR = (255, 80, 255)  # BGR magenta; distinct from local green/cyan.


def fused_action_label_text(fusion_prediction, args):
    """Return the fused top class text, including the fused gate state."""

    ranking = fusion_prediction.get("ranking") or []
    label = fusion_prediction.get("action_label")
    if label is None and ranking:
        label = ranking[0].get("label")
    try:
        label = int(label)
    except (TypeError, ValueError):
        return "no top class"

    label_text = prediction_label_text(label, args, max_chars=58)
    decision = str(fusion_prediction.get("decision", "class")).upper()
    if decision not in {"CLASS", "ACCEPTED"}:
        return f"{decision}: top {label_text}"
    return label_text
...
def draw_fusion_overlay(
    ...
    """Draw the latest two-robot fused action in a dedicated color block."""

    if not getattr(args, "fusion_topic", None):
        return

    color = FUSION_DISPLAY_COLOR
    cv2.putText(
        frame_bgr,
        "FUSED (rosbot_1 + rosbot_2)",
        (18, base_y),
    ...
    if fusion_prediction is None:
        text = "waiting for /har/final_action"
    else:
        try:
            age = max(0.0, float(fusion_age_sec))
        except (TypeError, ValueError):
            age = 0.0
        cycle = int(fusion_prediction.get("cycle_id", 0))
        score = float(fusion_prediction.get("action_score", 0.0))
        label_text = fused_action_label_text(fusion_prediction, args)
        if age > float(getattr(args, "fusion_stale_seconds", 15.0)):
            text = f"STALE {age:.1f}s | last {label_text} | cycle={cycle}"
        else:
            text = f"{label_text} score={score:.4f} | cycle={cycle} age={age:.1f}s"
    cv2.putText(
        frame_bgr,
        truncate_text(text, max_chars=108),
        (18, base_y + 24),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.60,
        color,
...
def draw_overlay(
    ...
    fusion_lines = 2 if getattr(args, "fusion_topic", None) else 0
    overlay_height = max(175, 135 + shown_rows * 25 + fusion_lines * 25 + 16)
    ...
    fusion_base_y = ranking_base_y + max(1, len(prediction.get("labels", []))) * 24 + 18
    draw_fusion_overlay(
        frame_bgr,
        fusion_prediction,
        fusion_age_sec,
        args,
        base_y=fusion_base_y,
    )
    if not args.hide_person_boxes:
        draw_person_boxes(
            frame_bgr,
...
def main(args=None, capture_factory=None):
    ...
    if getattr(args, "fusion_topic", None):
        print(
            f"  fusion_display: {args.fusion_topic} "
            f"(stale>{args.fusion_stale_seconds:g}s; rosbot_1 overlay)"
        )
    ...
    try:
        while True:
            ...
            if should_predict:
                ...
                publish_ros_prediction(prediction, args, cap)
                if args.headless:
                    print_prediction(display_prediction, args)

            if not args.headless:
                display_frame = resize_for_display(frame_bgr, args)
                fusion_prediction = getattr(cap, "latest_fusion_prediction", None)
                fusion_age_sec = None
                fusion_received = getattr(cap, "latest_fusion_received_monotonic", None)
                if fusion_received is not None:
                    fusion_age_sec = max(0.0, time.monotonic() - fusion_received)
                draw_overlay(
                    display_frame,
                    display_prediction,
                    frame_count,
