...
def build_ros_arg_parser():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "JSON VPOCLIP prediction topic. Defaults to "
            "<robot-namespace>/vpoclip/prediction."
        ),
    )
    parser.add_argument(
        "--fusion-topic",
        default=None,
        help=(
            "JSON fused-action topic to show in the GUI. When set, the latest "
            "two-view fusion is drawn as a separate colored overlay."
        ),
    )
    parser.add_argument(
        "--fusion-stale-seconds",
        type=float,
        default=15.0,
        help="Seconds after which the last fused result is marked stale in the GUI.",
    )
    parser.add_argument(
...
def validate_ros_args(args):
    ...
    if not args.prediction_topic.startswith("/"):
        raise ValueError("--prediction-topic must be an absolute ROS topic name.")
    if args.fusion_topic is not None and not args.fusion_topic.startswith("/"):
        raise ValueError("--fusion-topic must be an absolute ROS topic name.")
    if args.fusion_stale_seconds <= 0.0:
        raise ValueError("--fusion-stale-seconds must be positive.")
...
class RosImageSource:
    ...
    def __init__(self, args):
        ...
        self.latest_fusion_prediction = None
        self.latest_fusion_received_monotonic = None
        ...
        self.prediction_publisher = self.node.create_publisher(
            String, args.prediction_topic, 10
        )
        self.prediction_sequence = 0
        self.fusion_subscription = None
        if args.fusion_topic:
            self.fusion_subscription = self.node.create_subscription(
                String,
                args.fusion_topic,
                self._on_fusion,
                10,
    ...
    def _on_fusion(self, message):
        ...
        if not isinstance(payload, dict):
            return
        if payload.get("schema") != "har.vpoclip.fused_action.v1":
            return
        if "action_label" not in payload and not payload.get("ranking"):
            return
        self.latest_fusion_prediction = payload
        self.latest_fusion_received_monotonic = time.monotonic()
...
def parse_args(argv=None):
    ...
    if args.person_detection_frame_id is None:
        args.person_detection_frame_id = (
            f"{args.robot_namespace}/camera_color_optical_frame"
        )
    if args.prediction_topic is None:
        args.prediction_topic = f"{namespace_prefix}/vpoclip/prediction"
